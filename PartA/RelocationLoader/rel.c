...
{
 strcpy(bitmask,"");int i;
 for(i=0;i<strlen(hex);i++)
 {
  switch(hex[i])
  {
    case '0':strcat(bitmask,"0000");
             break;
...
void main()
{
 ...
 char type,progname[20],objcode[20],input[20],mask[20],sub[20];
 int psa,sa,addr,len,c=0;
 fscanf(f1,"%c %s %x %x",&type,progname,&psa,&len);
 printf("Enter the starting address\n");
 scanf("%x",&sa);
 printf("Loading program %s with starting address %x\n",progname,(psa+sa));
 fscanf(f1,"%s",input);
